...
VERSION_20_COLUMNS = {
    name: set(columns)
    for name, columns in VERSION_21_COLUMNS.items()
}
VERSION_20_COLUMNS["job_runs"] -= {
    "failure_class",
    "failure_reason",
}
...
VERSION_19_COLUMNS = {
    name: set(columns)
    for name, columns in VERSION_20_COLUMNS.items()
}
...
def migrate_20_to_21(
    connection: sqlite3.Connection,
) -> None:
    """Add durable failure classification for completed failed runs."""
    _validate_fingerprint(
        connection,
        VERSION_20_COLUMNS,
    )
    ...
    connection.execute(
        """
        ALTER TABLE job_runs
        ADD COLUMN failure_class TEXT NOT NULL DEFAULT 'NONE'
        CHECK(failure_class IN (
            'NONE',
            'LOCAL',
            'REMOTE',
